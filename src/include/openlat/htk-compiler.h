...
public:
  ...
  float acoustic;  // Acoustic likelihood of link
  float language;  // General language model likelihood of link
  std::vector<int> xscores; // xscores
...
public:
   ...
   HtkContext(istream* _is = &cin): scanner(0), is(_is), fst(0) {
      init_scanner();
   }
...
int HtkParse(HtkContext*);


/*
template <class A> class HtkCompiler {
 public:
  typedef A Arc;
